...
export default function PnLTable() {
  const { data, isLoading, error } = useQuery('pnl', getPortfolioPnl);

  if (isLoading) return <div className="p-4 text-center">Загрузка PnL...</div>;
  if (error) return <div className="p-4 text-center">Ошибка загрузки PnL</div>;
        ...
        <tbody>
          {data.map((item: any) => (
            <tr key={item.currency}>
              ...
              <td className="text-right" style={{ color: item.gainLoss >= 0 ? 'green' : 'red' }}>
