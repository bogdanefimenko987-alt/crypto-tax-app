...
router.get('/report/:year', (req: AuthRequest, res: Response) =>
  res.json({ events: [], summary: { totalProceeds: 0, totalCost: 0, totalGain: 0, totalTax: 0 } })
);
router.get('/report/:year/csv', (req: AuthRequest, res: Response) =>
  res.header('Content-Type', 'text/csv').send('date,currency,proceeds,costBasis,gainLoss,taxRate,taxAmount')
);
router.get('/report/:year/pdf', (req: AuthRequest, res: Response) =>
  res.json({ events: [], summary: { totalProceeds: 0, totalCost: 0, totalGain: 0, totalTax: 0 } })
);
router.get('/declaration/:year', (req: AuthRequest, res: Response) =>
